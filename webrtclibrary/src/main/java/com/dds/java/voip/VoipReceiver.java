...
public class VoipReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Utils.ACTION_VOIP_RECEIVER(context).equals(action)) {
            ...
            SkyEngineKit.init(new VoipEvent(context));
            boolean b = SkyEngineKit.Instance().startInCall(context, room, inviteId, audioOnly);
            if (b) {
                if (list.length == 1) {
