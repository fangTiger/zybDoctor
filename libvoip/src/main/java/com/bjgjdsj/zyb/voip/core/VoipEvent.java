...
public class VoipEvent implements ISkyEvent {
    ...
    public void shouldStartRing(boolean isComing) {
        if (isComing) {
            Uri uri = Uri.parse("android.resource://" + BaseApplication.getAppContext().getPackageName() + "/" + R.raw.incoming_call_ring);
            ringPlayer.play(BaseApplication.getAppContext(), uri, true, AudioManager.STREAM_RING);
        } else {
            Uri uri = Uri.parse("android.resource://" + BaseApplication.getAppContext().getPackageName() + "/" + R.raw.wr_ringback);
            ringPlayer.play(BaseApplication.getAppContext(), uri, true, AudioManager.STREAM_RING);
        }
    }
