...
public class PhotoView extends AppCompatImageView {
    ...
    private RectF mClip;
    private PhotoViewInfo mFromInfo;
...
//            mScaleCenter.set(detector.getFocusX(), detector.getFocusY());
    ...
    private class Transform implements Runnable {
        ...
        void withTranslate(int startX, int startY, int deltaX, int deltaY) {
            mLastTranslateX = 0;
            mLastTranslateY = 0;
            mTranslateScroller.startScroll(0, 0, deltaX, deltaY, mAnimDuring);
        }
    ...
    public PhotoViewInfo getInfo() {
        RectF rect = new RectF();
        int[] p = new int[2];
        getLocation(this, p);
        rect.set(p[0] + mImgRect.left, p[1] + mImgRect.top, p[0] + mImgRect.right, p[1] + mImgRect.bottom);
        return new PhotoViewInfo(rect, mImgRect, mWidgetRect, mBaseRect, mScreenCenter, mScale, mDegrees, mScaleType);
    }

    public static PhotoViewInfo getImageViewInfo(ImageView imgView) {
        int[] p = new int[2];
        ...
        return new PhotoViewInfo(rect, imgRect, widgetRect, baseRect, screenCenter, 1, 0, imgView.getScaleType());
    }
     ...
     */
    public void animaFrom(PhotoViewInfo info) {
        if (isInit) {
            reset();

            PhotoViewInfo mine = getInfo();
            ...
            if (info.mWidgetRect.width() < info.mImgRect.width() || info.mWidgetRect.height() < info.mImgRect.height()) {
                ...
                mTranslate.withClip(clipX, clipY, 1 - clipX, 1 - clipY, mAnimDuring / 3, c);
    ...
    public void animTo(PhotoViewInfo info, Runnable completeCallBack) {
        if (isInit) {
            mTranslate.stop();
            ...
            if (info.mWidgetRect.width() < info.mRect.width() || info.mWidgetRect.height() < info.mRect.height()) {
                ...
                postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        mTranslate.withClip(1, 1, -1 + cx, -1 + cy, mAnimDuring / 2, c);
                    }
                }, mAnimDuring / 2);
            }
