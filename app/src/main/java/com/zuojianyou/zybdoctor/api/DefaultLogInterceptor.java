...
import com.zuojianyou.zybdoctor.utils.ZipHelper;
...
public class DefaultLogInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        long t1 = BuildConfig.DEBUG ? System.nanoTime() : 0;
        Response originalResponse;
        try {
            originalResponse = chain.proceed(request);
        } catch (Exception e) {
            Log.e("HttpError","Http Error: " + e);
            throw e;
        }
        long t2 = BuildConfig.DEBUG ? System.nanoTime() : 0;
        ...
        if (responseBody != null && isParseable(responseBody.contentType())) {
            bodyString = printResult(request, originalResponse, BuildConfig.DEBUG);
        }
        ...
        if (logResponse) {
            ...
            if (responseBody != null && isParseable(responseBody.contentType())) {
                ...
                DefaultFormatPrinter.printFileResponse(TimeUnit.NANOSECONDS.toMillis(t2 - t1),
                    isSuccessful, code, header, segmentList,
                    message, url);
            }
        }
    ...
    public static String parseParams(Request request) {
        try {
            ...
            //return CharacterHandler.jsonFormat(
            // URLDecoder.decode(requestbuffer.readString(charset), convertCharset(charset)));
            return CharacterHandler.jsonFormat(requestbuffer.readUtf8());
        } catch (IOException e) {
            e.printStackTrace();
            return "{\"error\": \"" + e.getMessage() + "\"}";
        }
